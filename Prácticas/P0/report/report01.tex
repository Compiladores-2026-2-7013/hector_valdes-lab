...
Resultado : 28.274401
\end{lstlisting}
  \caption{Salida de el programa ejecutable}
\end{figure}
...
\begin{enumerate}[label=\alph*)]
  \item Generamos nuevamente un archivo \texttt{.i} con el nombre programaPI.i y notamos dos diferencias a primera vista. Veamos los siguientes extractos comprarando ambos archivos \textit{.i}.
    \begin{figure}[!h]
\begin{lstlisting}
int main ( void ) {
  printf ("Hola Mundo !\n");
...
\end{lstlisting}
\caption{\textit{Extracto de el original programa.i}}
\end{figure}
  \begin{figure}[!h]
\begin{lstlisting}
int main ( void ) {
...
\end{lstlisting}
\caption{\textit{Extracto de nuevo programa.i}}
\end{figure} 
\par
...
\item ¿Cambia en algo la ejecución final? \par
...
\end{lstlisting}
  \caption{Codigo con diferentes directivas}
\end{figure}

El codigo calcula el producto de dos constantes las cuales son modificadas con directivos de preprocesamiento. Los directivos que use fueron los siguientes:
\begin{enumerate}[label=\alph*)]
  \item \textbf{\#ifndef} \#ARG \par
    Ejecuta el codigo dentro de el cuerpo del if si el \#ARG no esta definido. Lo uso para asegurar que por lo menos haya una constante definida.
  \item \textbf{\#error} \#ARG \par
    Falla con el mensaje de error \#ARG en la etapa del preprocesamiento. Si no hay por lo menos una constante definida el programa no podira funcionar, es por eso que envio un error desde ese parte del proceso.
  \item \textbf{\#warning} \#ARG
En la etapa de preprocesado presenta una advertencia con elmensaje \#ARG. Esto lo hice para advertirle al usuario que estaremos usando el valor default de la constante dos, dado que no pudimos encontrar otro.
  \item \textbf{\#pragma} 
    Da instrucciones especificas en el proceso de compilacion, por ejemplo para que solo lea una vez cierto archivo, o que envie una advertencia. Esta directiva la usamos para dar un boost de seguridad al programador, lo cual es esencial para el funcionamiento del programa.
\end{enumerate}
